defmodule QasMicro.Worker.PurposeLogger do
  @moduledoc """
  """

  @behaviour :gen_event

  @type handler :: {atom, term}
  ...
  def handle_call(:path, %{handler: handler} = state) do
    {:ok, {:ok, handler}, state}
  end
  ...
  defp log_event(_level, _msg, _ts, _md, %{handler: nil} = state) do
    {:ok, state}
  end

  defp log_event(level, msg, ts, md, %{handler: handler} = state) do
