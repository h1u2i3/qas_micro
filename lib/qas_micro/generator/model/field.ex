defmodule QasMicro.Generator.Model.Field do
  ...
  defp handle_field_type(type, acc) do
    case type do
      ...
      type when type in [:text, :file, :files] -> QMap.put(acc, :type, :string)
      type when type in [:json] -> QMap.put(acc, :type, :map)
      type when type in [:jsons] -> QMap.put(acc, :type, {:array, :map})
