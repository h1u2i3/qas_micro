defmodule QasMicro.Api.Server do
  @moduledoc """
  ...
  #
  def run(app, middlewares, options) do
    pipeline = %Pipeline{
      event: :call,
      assigns:
        options
  ...
  def handle_success(_pipeline), do: Logger.info("success")

  def handle_error(_pipeline, reason), do: Logger.error("error: #{inspect(reason)}")
end
