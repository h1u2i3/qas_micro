  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def handle_cast({:start_task, {app, pipeline, middlewares}}, state) do
        task =
          Task.Supervisor.async_nolink(QasMicro.TaskSupervisor, fn ->
            pipeline
      ...
      def handle_info({ref, pipeline}, state) do
        ...
        QasMicro.Cache.remove({app, __MODULE__})
      ...
      def handle_info({:DOWN, ref, :process, _pid, reason}, state) do
        if app = find_app(state, ref) do
          # handle error info
          pipeline = QasMicro.Cache.lookup({app, __MODULE__})
